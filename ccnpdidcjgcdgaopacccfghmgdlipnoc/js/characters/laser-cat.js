...
function createLaserElement(eyeCenter, targetCenter) {
    ...
    Object.assign($laser.style, {
        width: `${getDistanceBetweenPoints(eyeCenter, targetCenter)}px`,
        height: `${height}px`,
        borderRadius: `4px`,
...
export default class LaserCat extends Character {
    ...
    async handleAutoTrigger($elem) {
        this.shootLaserAtTarget($elem, undefined);
    }
    ...
    handleDidClickTarget(e, $target) {
        ...
        this.shootLaserAtTarget($target, {
            x: e.clientX,
            y: e.clientY
        });
    }
     ...
     * @param {*} mousePosition
     */
    shootLaserAtTarget($target, mousePosition) {
        ...
        for (const $eye of this.eyes) {
            ...
            // Spawn the laser and remove target
        ...
        // Add explosion
        this.spawnExplosion(targetCenter).then();
        ...
        // there's a 30% chance that a rainbow will spawn.
...
defineCustomElement("laser-cat", LaserCat);